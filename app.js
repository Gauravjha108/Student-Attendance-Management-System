...
function initSearchForm() {
    ...
    form.addEventListener('submit', (e) => {
        ...
        const timerText = document.getElementById('search-time-taken');
        ...
        if (timerBadge && timerText) {
            ...
            timerText.textContent = `${duration.toFixed(3)} ms`;
        }
...
async function bubbleSortVisual(criteria) {
    ...
    const timerDisplay = document.getElementById('sort-timer-display');
    ...
    const updateTimer = () => {
        if (timerDisplay) {
            timerDisplay.textContent = `${(performance.now() - startTime).toFixed(2)} ms`;
        }
    };
    ...
    if (timerDisplay) timerDisplay.textContent = `${elapsed.toFixed(2)} ms`;
...
async function mergeVisual(students, left, mid, right, criteria, stats) {
    ...
    const updateTimer = () => {
        if (timerDisplay && stats.startTime) {
            timerDisplay.textContent = `${(performance.now() - stats.startTime).toFixed(2)} ms`;
        }
    };
...
async function triggerMergeSort(criteria) {
    ...
    const startTime = performance.now();
    const stats = { comparisons: 0, startTime };
...
// Spotlight Hover mouse tracking
...
// Particle Background Physics engine (CPU lightweight)
...
function initCSVExport() {
    const exportBtnDir = document.getElementById('btn-export-csv');
    const exportBtnRep = document.getElementById('btn-export-csv-rep');
    
    if (exportBtnDir) {
        exportBtnDir.addEventListener('click', exportToCSV);
    }
    if (exportBtnRep) {
        exportBtnRep.addEventListener('click', exportToCSV);
    }
}

// Export logic to generate downloadable file
function exportToCSV() {
    ...
    csvContent += "Roll Number,Name,Department,Total Classes,Attended Classes,Attendance Percentage\n";
    
    students.forEach(s => {
        const pct = s.totalClasses > 0 ? ((s.attendedClasses / s.totalClasses) * 100).toFixed(2) : "0.00";
        const nameEscaped = s.name.includes(',') ? `"${s.name}"` : s.name;
        csvContent += `${s.rollNo},${nameEscaped},${s.department},${s.totalClasses},${s.attendedClasses},${pct}%\n`;
    });
    
    const encodedUri = encodeURI(csvContent);
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    link.setAttribute("download", `student_attendance_report_${new Date().toISOString().slice(0,10)}.csv`);
